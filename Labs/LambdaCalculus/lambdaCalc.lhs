...
Because, if the first two values are tru, that is the case when we will have tru tru x type expression. tru in this case return first value which is tru, otherwise, it will always return the second value which is fls
...
> testOps1 = transBool $ orr tru (nott tru)
> testOps2 = transBool $ nott (orr fls fls)
